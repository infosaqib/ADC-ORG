...
# This script processes all pages from pages.txt
...
# Process all entries from pages.txt
$processedCount = 0
$skippedCount = 0
...
foreach ($entry in $pagesContent) {
    ...
    if ($info) {
        ...
        if (Test-Path $outputPath) {
            Write-Host "  Skipping (already exists): $outputPath" -ForegroundColor Yellow
            $skippedCount++
            continue
        }
        ...
        # STEP 6: Replace URLs in meta tags and schema (use lowercase cityFile to match pages.txt exactly)
        $lowerCityFile = $cityFile.ToLower()
        # Replace og:url
        $template = $template -replace 'property="og:url" content="https://services\.armydogcenter\.org\.pk/[^"]+"', "property=`"og:url`" content=`"https://services.armydogcenter.org.pk/$lowerCityFile.php`""
        # Replace canonical
        $template = $template -replace 'rel="canonical" href="https://services\.armydogcenter\.org\.pk/[^"]+"', "rel=`"canonical`" href=`"https://services.armydogcenter.org.pk/$lowerCityFile.php`""
        # Replace schema markup URLs
        $template = $template -replace '"@id":"https://services\.armydogcenter\.org\.pk/[^"]+#webpage"', "`"@id`":`"https://services.armydogcenter.org.pk/$lowerCityFile.php#webpage`""
        $template = $template -replace '"url":"https://services\.armydogcenter\.org\.pk/[^"]+"', "`"url`":`"https://services.armydogcenter.org.pk/$lowerCityFile.php`""
        
        # STEP 7: Fix og:site_name (should be just organization name, not full title)
        ...
        # STEP 8: Update schema description with city name
        ...
        # STEP 9: Update all text references to city in content
...
Write-Host "`nPage creation completed!" -ForegroundColor Cyan
Write-Host "  Created: $processedCount pages" -ForegroundColor Green
